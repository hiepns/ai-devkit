...
1. **Gather Context** — If not already provided, ask for: feature name (kebab-case, e.g., `user-authentication`), brief feature/branch description, planning doc path (default: latest matching `{{docsDir}}/planning/YYYY-MM-DD-feature-{name}.md`; fallback `{{docsDir}}/planning/feature-{name}.md`), and any supporting docs (design, requirements, implementation).
