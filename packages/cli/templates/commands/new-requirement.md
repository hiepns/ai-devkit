...
3. **Initialize Feature Documentation Structure** — Run `npx ai-devkit@latest docs init-feature {name}` and fill the returned paths. It uses configured phases. If unavailable, copy each configured phase `README.md` to `{{docsDir}}/{phase}/feature-{name}.md`, preserving frontmatter and headings.
