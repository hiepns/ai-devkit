...
1. **Gather Context** — If not already provided, ask for: feature/branch description, relevant design doc(s), known constraints or risky areas, and tests run. Prefer latest matching `{{docsDir}}/design/YYYY-MM-DD-feature-{name}.md`; if none exists, use `{{docsDir}}/design/feature-{name}.md`. Run `git status` and `git diff --stat` to identify modified files.
